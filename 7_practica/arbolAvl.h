...
void ArbolBinarioAVL<key>::rotate_DI(NodoBinario<key>* &nodo) {
  ...
  if (nodo1 == nullptr) {
    throw std::invalid_argument("nodo1 no puede ser nullptr");
  }
  ...
  if (nodo2 != nullptr) {
    ...
    // nodo2 es nullptr, realizar una rotación simple a la derecha
    nodo->setRight(nodo1->getRight());
    nodo1->setRight(nodo);
    if (nodo1->getBalance() == -1) {
      nodo->setBalance(1);
      nodo1->setBalance(0);
    } else if (nodo1->getBalance() == 0) {
      nodo->setBalance(0);
      nodo1->setBalance(0);
    } else { // nodo1->getBalance() == 1
      nodo->setBalance(0);
      nodo1->setBalance(-1);
    }
    nodo = nodo1;
