...
void help() {
  ...
  std::cout << "-trace <true|false>, indica si se desea mostrar el árbol tras cada inserción\n";
}

void readArgs(std::string& arbolType, std::string& introduceData, std::string& file, int& numElements, int argc, char* argv[], std::string& trace) {
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 4 || argc > 9 || arg1 == "-h") {
    help();
    return 0;
  }
  std::string arbolType = "", introduceData = "", file = "", trace = "";
  int numElements = 0;
  readArgs(arbolType, introduceData, file, numElements, argc, argv, trace);
  if (arbolType == "abe") {
  ...
  else if (arbolType == "avl") {
    ArbolBinarioAVL<Dni> tree(trace == "true");
    useTree(tree, introduceData, numElements, file);
  }
  else {
